...
最近好奇使用了一下 NextJS 做了几个小的前端项目。用了一段时候发现，这个框架极有意思。
...
这里有很多奥妙，而也是我认为 NextJS 最大的一个坑。这里的根源就是在于 NextJS 会自动拆分代码，一部分运行在服务器，另一部分运行在客户端。

所以，在落笔代码之前，一定要对你的项目有个大体的构架：哪些功能用 Server 组件，哪些用客户端渲染的 Client 组件，哪些地方用 ServerAction，哪些地方声明 API Routes。

否则，你的项目在运行时会遇到大量错误，很多要重构才能解决。

对于这个问题，我有以下四点总结：

- **优先使用服务端渲染的 Server 组件**（`'use server'`）：将最大化 NextJS 的优势，也可以获得更好的性能。一般来说，数据展示、单向的表单提交功能都适合用服务端渲染。当然 NextJS 默认也是服务端渲染。
